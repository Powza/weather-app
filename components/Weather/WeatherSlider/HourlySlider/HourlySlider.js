        ...
        <MicroModal
          ...
          trigger={handleOpen => (
            <div onClick={handleOpen} target="_blank" className={[styles.slide, styles["slide-modal-btn"]].join(" ")}>
              {moment().format("ddd") === moment.unix(item.time).format("ddd") ? (
                <div className={styles.date}>{moment.unix(item.time).format("h a")}</div>
              ) : (
          ...
          children={handleClose => (
            <>
              <div className={styles["modal__top"]}>
                ...
                {moment().format("ddd") === moment.unix(item.time).format("ddd") ? (
                  <h2 className={styles.date}>{moment.unix(item.time).format("h a")}</h2>
                ) : (
                  <h2 className={styles.date}>{moment.unix(item.time).format("ddd h a")}</h2>
                )}
                <div className="row">
